...
const OurMenu = () => {
  ...
  const scrollTabs = (direction) => {
    ...
    const totalWidth = Array.from(container.children).reduce(
      (sum, child) => sum + child.clientWidth + 3, // Account for 3px gap
      0
    );
    ...
    if (direction === 'left') {
      ...
      const currentScroll = visibleStartIndex * (calculateCardWidth(menuHeadItems[0].title) + 3);
  ...
  const isRightDisabled = () => {
    ...
    return visibleStartIndex * (calculateCardWidth(menuHeadItems[0].title) + 3) + containerWidth >= totalWidth;
  };
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative z-10 bg-[#0b1315] py-10 px-6 pt-[160px]">
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="flex justify-center items-start mb-4 gap-2">
            <button
              onClick={() => scrollTabs('left')}
              disabled={isLeftDisabled}
              className={`p-2 mt-[2px] rounded-full ${isLeftDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-700'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="#C9AB81">
            ...
            <div className="flex-1 overflow-hidden">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full ">
                <TabsList 
                  ...
                  className="flex transition-transform duration-300 gap-[3px]  h-[60px]  px-2"
                  style={{
                ...
                <div className="text-center mt-16 mb-5">
                  ...
                  <h2 className="text-4xl md:text-5xl text-[#f3c97c] tracking-wide uppercase mb-4 font-bold">
                    {menuHeadItems.find(item => `menu-${item.id}` === activeTab)?.title || "Menu"}
                  </h2>
            ...
            <button
              ...
              className={`p-2 mt-[2px] rounded-full ${isRightDisabled() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-700'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="#C9AB81">
